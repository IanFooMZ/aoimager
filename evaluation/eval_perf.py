...
if processor_rank == 0:
	...
	#! TODO: Go to the method definition and change the length. I've only turned it off so that debugging won't take forever.
	...
	ground_truth_training = training_samples.targets
	ground_truth_validation = validation_samples.targets

	# Generate the zones at the detector plane that will correspond to each of the classes.
	target_intensity_distributions = utility.target_classes_to_zones(training_samples.classes, p.data['img_dimension'])
	...
	if run_mode == 'opt':
		...
		# Define NN model
		nn_model = networks.identity_net()		 # Literally no NN architecture to optimize at all
		...
		for Q_idx in range( 0, num_Q_iterations ):
			...
			for nn_epoch in range( 0, p.optimization['num_nn_epochs_per_photonic_epoch'] ):
				...
				for nn_batch_idx in range( 0, number_batches ):
					...
					ground_truth_output = torch.squeeze( training_samples.targets[ batch_start_idx : batch_end_idx ] )
			...
			training_error[ Q_idx ] = eval_model_by_dataset( training_Ek, training_samples.targets )
			print(f'Evaluating Training Error for Q_abs={Qabs}: {training_error[Q_idx]:.5f}')
			validation_error[ Q_idx ] = eval_model_by_dataset( validation_Ek, validation_samples.targets )
			...
			def function_nlopt(x, gradn):
				...
				print(f'Idx_diffraction is {iteration_counter_diffraction} and Photonic Batch Idx_Diffraction is {photonic_batch_idx_diffraction}.')
